...
rr_impute_missing <- function(x) {
    x %>%
        ...
        relocate(all_of(c("Study", "Reference", "Note")), .after=last_col())
}
...
get_ERR_from_ORRRSIRIRR <- function(x) {
    ...
    ## remove any prior ERR results
    x$ERR[]      <- NA_real_
    x$ERR_CIlo[] <- NA_real_
    x$ERR_CIup[] <- NA_real_
    x$CI_width_ERR[!idx] <- NA_real_
...
get_all_ERR_from_RR <- function(x, n_repl=1000, cor_intra=0.5) {
    ...
    d_ERR_ext <- x %>%
        ...
        select(auth_year, Study, Reference, CI_width_ERR, ERR, ERR_CIlo, ERR_CIup) %>%
        mutate(from="ext") %>%
        unique()
    ...
    d_ERR_long <- bind_rows(d_ERR_org, d_ERR_lm, d_ERR_gls, d_ERR_ext) %>%
...
## original, gls, lm, external, none
get_used <- function(val_org, val_gls, val_lm, val_ext) {
    used <- if_else(is.na(val_org),
                    if_else(is.na(val_gls),
                            if_else(is.na(val_lm),
                                    if_else(is.na(val_ext),
                                            "none",
                                            "ext"),
                                    "lm"),
                            "gls"),
    ...
    factor(used, levels=c("org", "gls", "lm", "ext", "none"))
}
