...
Computational geometry is carried out mainly using the \texttt{CGAL} library \cite{CGAL2022} via package \texttt{RcppCGAL} \cite{Dunipace2022} used in package \texttt{MeshUtils} \cite{Wollschlager2026}. Distance maps, isotropic remeshing, and mesh smoothing are based on the \texttt{VCG} library \cite{VCG2022} via package \texttt{Rvcg} \cite{Schlager2017}.
